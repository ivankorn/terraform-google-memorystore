...
# Running hadolint in Docker in case it fails. To be addressed/re-written here:
# https://github.com/terraform-google-modules/terraform-google-project-factory/issues/252
function check_docker() {
  echo "Running hadolint on Dockerfiles"
  # shellcheck disable=SC2044
  for dockerfile in $(find . -name "Dockerfile" ! -path '*/.terraform/*'); do
    # Check if hadolint is installed
    # shellcheck disable=SC2230
    which hadolint &>/dev/null
